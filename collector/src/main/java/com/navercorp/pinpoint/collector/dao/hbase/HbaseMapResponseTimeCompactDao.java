...
import com.navercorp.pinpoint.collector.dao.hbase.statistics.PingColumnName;
import com.navercorp.pinpoint.collector.dao.hbase.statistics.ResponseColumnCompactName;
 ...
 */
@Repository
public class HbaseMapResponseTimeCompactDao implements MapResponseTimeCompactDao {
    ...
    @Autowired
    public HbaseMapResponseTimeCompactDao(HbaseOperations2 hbaseTemplate,
                                          ...
                                          @Qualifier("selfCompactBulkIncrementer") BulkIncrementer bulkIncrementer) {
    ...
    @Override
    public void received(String applicationName, ServiceType applicationServiceType, int elapsed, boolean isError) {
        ...
        final ColumnName columnName = new ResponseColumnCompactName(HbaseColumnFamily.SelfStatMapCompact.QUALIFIER_RESPONSE, slotNumber);
        if (useBulk) {
            TableName mapStatisticsSelfTableName = descriptor.getTableName();
            bulkIncrementer.increment(mapStatisticsSelfTableName, selfRowKey, columnName);
        } else {
            final byte[] rowKey = getDistributedKey(selfRowKey.getRowKey());
            // column name is the name of caller app.
            byte[] columnNameBytes = columnName.getColumnName();
            increment(rowKey, columnNameBytes, 1L);
        }
    }

    @Override
    public void ping(String applicationName, ServiceType applicationServiceType, String agentId) {
        Objects.requireNonNull(applicationName, "applicationName");
        ...
        final long rowTimeSlot = timeSlot.getTimeSlot(acceptedTime);
        final RowKey selfRowKey = new CallRowKey(applicationName, applicationServiceType.getCode(), rowTimeSlot);
        final ColumnName columnName = new PingColumnName(HbaseColumnFamily.SelfStatMapCompact.QUALIFIER_PING, agentId);
        if (useBulk) {
            TableName tableName = descriptor.getTableName();
            bulkIncrementer.increment(tableName, selfRowKey, columnName);
        } else {
            final byte[] rowKey = getDistributedKey(selfRowKey.getRowKey());
            // column name is the name of caller app.
            byte[] columnNameBytes = columnName.getColumnName();
