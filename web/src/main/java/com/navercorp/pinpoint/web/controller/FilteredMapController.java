...
public class FilteredMapController {
    ...
    public FilterMapWrap getFilteredServerMapDataMadeOfDotGroup(
        ...
        final LimitedScanResult<List<TransactionId>> limitedScanResult = defaultFilteredMapService.selectTraceIdsFromApplicationTraceIndex(applicationName, range, limit);
        ...
        ApplicationMap map = defaultFilteredMapService.selectApplicationMapWithScatterData(limitedScanResult.getScanData(), originalRange, scannerRange, xGroupUnit, yGroupUnit, filter, viewVersion);
    ...
    public FilterMapWrap getFilteredServerMapDataMadeOfDotGroupV2(
        ...
        final Filter<List<SpanBo>> filter = filterBuilder.build(filterText, filterHint);
        final Range range = Range.newRange(from, to);
        final LimitedScanResult<List<TransactionId>> limitedScanResult = compactFilteredMapService.selectTraceIdsFromApplicationTraceIndex(applicationName, range, limit);
        ...
        FilterMapWrap mapWrap = new FilterMapWrap(map, LoadHistogramFormat.V2);
        mapWrap.setLastFetchedTimestamp(lastScanTime);
        return mapWrap;
    }
}
